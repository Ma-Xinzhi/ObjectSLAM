...
public:
    ...
    void AddObjectObservation(const Object& Ob);
    bool EraseObjectObservation(int label);
    std::map<int, Objects> GetAllObjectObservations() const { return mObjectObservations; };
...
private:
    ...
    std::map<int, Objects> mObjectObservations; //地图中当前所有可能构成物体的观测，将语义类别作为关键字
