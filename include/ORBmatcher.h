...
public:
    ...
    int SearchByProjection(std::shared_ptr<Frame> pF, const std::vector<std::shared_ptr<MapPoint>> &vpMapPoints, float th=3);

    // Project MapPoints tracked in last frame into the current frame and search matches.
    // Used to track from previous frame (Tracking)
    int SearchByProjection(std::shared_ptr<Frame> pCurrentFrame, std::shared_ptr<Frame> pLastFrame, float th, bool bMono);

    // Project MapPoints seen in KeyFrame into the Frame and search matches.
    // Used in relocalisation (Tracking)
    int SearchByProjection(std::shared_ptr<Frame> pCurrentFrame, std::shared_ptr<KeyFrame> pKF, const std::set<std::shared_ptr<MapPoint>> &sAlreadyFound, float th, int ORBdist);
    int SearchByProjection(std::shared_ptr<Frame> pCurrentFrame, std::shared_ptr<KeyFrame> pKF, float th=3);
...
//    int SearchByProjection(std::shared_ptr<KeyFrame> pKF, cv::Mat Scw, const std::vector<std::shared_ptr<MapPoint>> &vpPoints, std::vector<std::shared_ptr<MapPoint>> &vpMatched, int th);
    ...
    int SearchForTriangulation(std::shared_ptr<KeyFrame> pKF1, std::shared_ptr<KeyFrame> pKF2, const Eigen::Matrix3d& F12,
    ...
    int Fuse(std::shared_ptr<KeyFrame> pKF, const std::vector<std::shared_ptr<MapPoint>> &vpMapPoints, float th=3.0);
